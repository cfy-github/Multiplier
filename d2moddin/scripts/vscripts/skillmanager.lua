...
function skillManager:RemoveAllSkills(hero)
    ...
    if not currentSkillList[hero] then
		...
		Log('heroClass: ' .. heroClass)

        -- Grab the skills
        local skills = self:GetHeroSkills(heroClass)
		Log('RemoveAllSkills: grab skills')
		PrintTable(skills)
		Log('------------')
...
function skillManager:ApplyBuildMultiplier(hero)
    -- Ensure the hero isn't nil
	Log('ApplySimpleBuild')
    if hero == nil then return end
	Log('hero not null')
    -- Remove all the skills from this hero
    self:RemoveAllSkills(hero)
    ...
    for i=1,12 do
        ...
        if v then
            ...
            currentSkillList[hero][abNum] = mab
...
function skillManager:ApplyBuildFactor(hero, factor)
	Log('ApplyBuildFactor')
	build = self:GetHeroSkills(hero, factor)
	PrintTable(build)
	self:ApplySimpleBuild(hero, build)
end
